      ...
      <div className="w-full max-w-[440px] relative z-10">
        ...
        <div className="bg-white/70 dark:bg-slate-900/40 backdrop-blur-md border border-white/80 dark:border-slate-800/80 rounded-[32px] p-8 shadow-[0_24px_70px_rgba(0,0,0,0.02)] dark:shadow-[0_24px_70px_rgba(0,0,0,0.4)] transition-all duration-300">
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 gap-3 p-1.5 bg-slate-100/80 dark:bg-slate-950/60 rounded-2xl border border-slate-200/40 dark:border-slate-800/40">
              {[
                { id: "OPD", label: "Outpatient", icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" },
                { id: "IPD", label: "Inpatient", icon: "M19 16v1a1 1 0 01-1 1H6a1 1 0 01-1-1v-1m14-4v2m-14-2v2m14-5V7a2 2 0 00-2-2H6a2 2 0 00-2 2v3m16 0h-2M4 10h2m11 4H7" }
              ].map((m) => (
                <button
                  key={m.id}
                  type="button"
                  onClick={() => { setModule(m.id); setError(""); }}
                  className={`flex flex-col items-center justify-center py-3 px-4 rounded-xl font-semibold text-sm transition-all duration-300 ${
                    module === m.id
                ...
                >
                  ...
                  <span className="text-xs font-bold tracking-wide">{m.id}</span>
                  <span className="text-[10px] font-medium opacity-60 font-mono tracking-tight mt-0.5">{m.label}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="mt-6 pt-5 border-t border-slate-100 dark:border-slate-800/60">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="bg-slate-50 dark:bg-slate-950/40 border border-slate-100 dark:border-slate-800/40 rounded-xl p-2.5 transition-colors">
                <div className="text-slate-700 dark:text-slate-300 font-bold mb-0.5 flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-blue-500"></span> Receptionist
                </div>
                <div className="text-slate-400 dark:text-slate-500 font-mono text-[11px]">receptionist / 123</div>
              </div>
              <div className="bg-slate-50 dark:bg-slate-950/40 border border-slate-100 dark:border-slate-800/40 rounded-xl p-2.5 transition-colors">
                <div className="text-slate-700 dark:text-slate-300 font-bold mb-0.5 flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-teal-500"></span> Medical Doctor
                </div>
                <div className="text-slate-400 dark:text-slate-500 font-mono text-[11px]">doctor / 123</div>
              </div>
            </div>
          </div>
        </div>
