...
// NEW FILE — profile view + password change. Wired in for Doctor first
// (see App.jsx / Sidebar.jsx), but works for any logged-in role since it
// only reads from useAuth() and calls a generic /auth/change-password route.
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 capitalize">{user.role} · {user.module}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <InfoRow icon={ShieldCheck} label="Role" value={user.role} capitalize />
          <InfoRow icon={Layers} label="Assigned Modules" value={(user.modules || []).join(", ")} />
        </div>
      </div>
  ...
  return (
    <div className="flex items-start gap-3 px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/40">
      ...
      <div className="min-w-0">
        ...
        <p className={`text-sm font-medium text-slate-800 dark:text-white truncate ${capitalize ? "capitalize" : ""}`}>
          {value || "—"}
        </p>
      </div>
    </div>
