  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        <button onClick={onBack} className="text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white text-sm font-medium transition-colors">
          ← Back
        </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 max-w-6xl mb-4">
        ...
        <SectionCard title="Admission Information" icon="🛏️">
          <div className="grid grid-cols-2 gap-3 text-sm">
            {[
          ...
          {p.notes && (
            <p className="text-slate-500 dark:text-slate-400 text-xs mt-3 leading-relaxed border-t border-slate-100 dark:border-slate-800 pt-3">
              {p.notes}
            </p>
          )}
        </SectionCard>
      ...
      <div className="max-w-6xl mb-4">
        <SectionCard title="Admission Items" icon="🧴">
          <div className="grid grid-cols-3 gap-4">
            {[
              { label: "Oil", val: p.oil, icon: "🫙" },
              { label: "Protein", val: p.protein, icon: "💪" },
              { label: "Syrup", val: p.syrup, icon: "🍯" },
            ].map(item => (
      ...
      <div className="max-w-6xl">
        <SectionCard title="Documents" icon="📎">
          {!readOnly && (
            <div className="flex gap-3 mb-4 flex-wrap">
              ...
              <button
                ...
                className="bg-violet-50 dark:bg-violet-500/20 text-violet-700 dark:text-violet-400 border border-violet-200 dark:border-violet-500/30 px-4 py-2 rounded-xl text-sm font-medium hover:bg-violet-100 dark:hover:bg-violet-500/30 transition-colors"
              >
                📤 Upload Document
              </button>
          ...
          {(!p.documents || p.documents.length === 0) ? (
            <div className="text-center py-8 text-slate-400 dark:text-slate-500 text-sm">No documents uploaded yet.</div>
          ) : (
              ...
              {p.documents.map(doc => (
                <div key={doc.id} className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
                  {isImage(doc.fileType) ? (
                    ...
                    <a href={doc.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center h-28 text-4xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                      📄
                    </a>
                  )}
                  <div className="p-2">
                    ...
                    {!readOnly && (
                      <button
                        ...
                        className="text-red-500 dark:text-red-400 text-xs hover:text-red-600 dark:hover:text-red-300 transition-colors mt-1"
                      >
                        Delete
                      </button>
                    )}
